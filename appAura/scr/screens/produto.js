...
// Cores
const MAIN_PINK = "#ff69b4"; // Rosa vibrante para destaque
const LIGHT_PINK = "#FFE4E8"; // Rosa bem claro para fundo
const LIGHT_BG = "#fff";
const DARK_TEXT = "#333";
...
// 🎯 CONFIGURAÇÃO PARA A TELA DE OFERTAS
const OFFERS_TITLE = "Ofertas de Make";
// 🚨 AJUSTE O ID: Defina o ID da categoria que contem as suas ofertas
...
const ProductCard = ({ product, onCardPress }) => {
    ...
    // Lógica de Oferta Simulação: Produtos com preço abaixo de R$50 têm 20% OFF
    const isOffer = product.price < 50; 
    const discount = isOffer ? '20% OFF' : null;
    ...
    return (
        <TouchableOpacity style={productCardStyles.productCard} onPress={() => onCardPress(product)}>
            ...
            {/* Tag de Oferta */}
            {discount && (
                <View style={productCardStyles.offerTag}>
                    <Text style={productCardStyles.offerTagText}>{discount}</Text>
                </View>
            )}
            ...
            <View style={productCardStyles.productDetails}>
                ...
                <View style={productCardStyles.priceContainer}>
                    {isOffer && (
                         // Preço Antigo (simulado: 125% do preço atual)
...
// ======================================================
// TELA OFERTAS DE MAKE (Exportada)
// ======================================================
export default function OfertasMakeScreen() {
    const navigation = useNavigation();
    ...
    const fetchCategoryProducts = async () => {
        ...
        const { data, error } = await supabase
            .from("products") 
            .select("*")
            // FILTRO DE CATEGORIA: OFFERS_CATEGORY_ID
            .eq('category_id', OFFERS_CATEGORY_ID); 

        if (!error) {
            setProducts(data);
        } else {
            console.error(`Erro ao buscar produtos de Ofertas:`, error.message);
            Alert.alert("Erro de Carga", "Não foi possível carregar as ofertas.");
        }
        setLoading(false);
    };
    ...
    return (
        <View style={screenStyles.container}>
            ...
            <View style={screenStyles.header}>
                ...
                {/* TÍTULO CENTRALIZADO */}
                <Text style={screenStyles.headerTitle}>{OFFERS_TITLE}</Text> 
            ...
            {loading ? (
                <View style={screenStyles.loadingContainer}>
                    ...
                    <Text style={screenStyles.loadingText}>Carregando ofertas...</Text>
                </View>
            ) : products.length === 0 ? (
                <View style={screenStyles.emptyContainer}>
                    <Ionicons name="pricetags-outline" size={50} color="#ccc" />
                    <Text style={screenStyles.emptyText}>Nenhuma oferta de Make encontrada.</Text>
                    <Text style={screenStyles.emptySubtext}>Verifique se o `OFFERS_CATEGORY_ID` ({OFFERS_CATEGORY_ID}) está correto no Supabase.</Text>
                </View>
            ) : (
                <FlatList
                    data={products}
...
const screenStyles = StyleSheet.create({
    ...
    placeholder: {
        width: 38, // Tamanho para balancear o backButton
    },
    loadingContainer: {
        flex: 1,
    ...
    listContentContainer: {
        paddingHorizontal: HORIZONTAL_PADDING,
        // ✅ Ajuste para a Tab Navigation (Conforme sua instrução salva)
        paddingBottom: 100, 
    },
    row: {
        justifyContent: 'space-between',
...
const toastStyles = StyleSheet.create({
    container: {
        position: 'absolute',
        // Posição para ficar acima da Tab Navigation (lembrando da sua instrução salva)
        bottom: 100, 
        left: '5%',
        right: '5%',
